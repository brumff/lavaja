...
@Service
public class ContratarServicoServiceOld {
    ...
    /*public int calcularFila(int index, List<ContratarServicoModel> list) {
        ...
        int tempoTotal = 0;
        var objetosNaFrente = list.subList(index, list.size());
        
        for (var model : objetosNaFrente) {
            if (model.getServico().getLavacarId().equals(lavaCar.getId()) && !model.isDeleted()) {
                var tempoServico = model.getServico().getTempServico() == null ? 0
                        : model.getServico().getTempServico();
                System.out.println(model.getId());


                var tempoDeServico = ChronoUnit.MINUTES.between(model.getDataServico(), LocalDateTime.now());
                
                if (model.getStatusServico() == StatusServico.AGUARDANDO) {
                    tempoTotal += tempoDeServico;
                    continue;
