...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        if (lavacar != null) { // Se encontrou um usuário do tipo lavacar, retorna as informações dele
            return new UserSS(lavacar.getId(), lavacar.getEmail(),  lavacar.getSenha(), lavacar.getPerfis(), lavacar.getNome());
        } else { // Se encontrou um usuário do tipo dono de carro, retorna as informações dele
            return new UserSS(donocarro.getId(), donocarro.getEmail(), donocarro.getSenha(), donocarro.getPerfis(), donocarro.getNome());
        }
